...
class DataTransformation:
    ...
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            if "obesity" in df.columns:
                df["obesity"] = df["obesity"].apply(lambda x: 1 if str(x).lower() in ["1", "yes", "true", "y"] else 0)
            elif "bmi" in df.columns:
                df["obesity"] = df["bmi"].apply(lambda x: 1 if float(x) > 29 else 0)

            # Compute hypertension if needed
            ...
            # -----------------------------
            # FIXED BLOCK: Age handling
            # -----------------------------
            if "age_category" in df.columns:
                # Convert string bucket → numeric encoding
                ...
                raise AutoCareException("❌ Dataset must contain either 'age_category' or 'age' column.", sys)

            # Boolean normalization
            for col in ["heart_attack_history", "chest_pain", "smoking", "alcohol", "physactivity", "highchol"]:
            ...
            # Drop unused fields
            df.drop(
                columns=[c for c in ["height", "weight", "bmi", "systolic", "diastolic", "Unnamed: 0"] if c in df.columns],
                inplace=True,
            )
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        try:
            ...
            if missing:
                raise AutoCareException(f"Missing required heart feature(s): {missing}", sys)
